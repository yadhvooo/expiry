...
export default function LoginPage({ setView }) {
  const { login, fillDemoCustomer } = useAuth();
  ...
  async function handleDemoLogin() {
    ...
    setError(null);
    try {
      await fillDemoCustomer();
      setView('home');
    } catch (err) {
      setError(err.message || 'Failed to sign in with demo customer.');
    } finally {
      setIsSubmitting(false);
    }
  }
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full border border-stone-200 shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-stone-500">Access your discounted food rescues and pickup orders</p>
        </div>
        ...
        <div className="pt-4 border-t border-stone-100 space-y-2">
          ...
          <button
            ...
            disabled={isSubmitting}
            className="w-full p-2.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold text-xs border border-emerald-200 transition-colors flex items-center justify-center gap-2"
          >
            <UserCheck className="w-4 h-4 text-emerald-600" />
            <span>Sign In as Demo Customer (Rahul Verma)</span>
          </button>
        </div>
